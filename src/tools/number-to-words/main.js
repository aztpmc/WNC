...
import { extractAmountsFromCsv, buildResultsCsv, neutralizeFormulaInjection } from './csv.js';
...
// Every language's actual "Amount" column header, sourced directly from
// the same dictionaries buildResultsCsv() uses to WRITE that header --
// not a hand-duplicated copy of the translated strings. Re-importing a
// CSV this app exported must work regardless of which language was
// active when it was exported, or which language is active now.
...
// "Copy Results" is meant to be pasted straight into a spreadsheet, same
// as the CSV export -- so every field goes through the same formula-
// injection neutralization as buildResultsCsv()'s csvEscape(), not just
// the CSV path. See neutralizeFormulaInjection() in csv.js for why.
function copyBulkResults() {
  if (!lastBulkResult || !lastBulkResult.rows.length) return;
  const lines = lastBulkResult.rows.map((row) => {
    if (!row.ok) return row.line + '\t' + neutralizeFormulaInjection(row.raw) + '\t' + bulkErrorMessage(row);
    const words = (lang === 'ar') ? row.arabic : row.english;
    return row.line + '\t' + neutralizeFormulaInjection(row.formattedWithCode) + '\t' + words;
  });
...
/* ----------------------------------------------------------------------
   ...
   ----------------------------------------------------------------------
   Import parses the file into the same newline-per-amount text the
   textarea already accepts and hands it to the existing runBulkConvert(),
   rather than a second amount-batch code path. Export builds a CSV from
   whatever the results table is currently showing -- so it always matches
   what the user sees, in the currently active interface language.
   ---------------------------------------------------------------------- */
...
function importBulkCsvFile(file) {
  ...
  reader.onload = () => {
    const amounts = extractAmountsFromCsv(String(reader.result), AMOUNT_HEADER_ALIASES);
    if (amounts === '') {
      bulkInput.value = '';
      lastBulkResult = null;
      renderBulkResults();
      bulkSummary.textContent = t().bulkCsvEmpty;
      bulkSummary.classList.add('has-errors');
      return;
    }
    bulkInput.value = amounts;
    runBulkConvert();
    const count = amounts.split('\n').length;
    bulkSummary.textContent = t().bulkCsvImported(count) + ' ' + bulkSummary.textContent;
  };
  reader.onerror = () => {
    lastBulkResult = null;
    renderBulkResults();
    bulkSummary.textContent = t().bulkCsvReadError;
    bulkSummary.classList.add('has-errors');
  };
...
function exportBulkResultsCsv() {
  if (!lastBulkResult || !lastBulkResult.rows.length) {
    bulkSummary.textContent = t().bulkNoResults;
    bulkSummary.classList.add('has-errors');
    return;
  }
  const headers = [t().bulkColLine, t().bulkColAmount, t().bulkColNumeric, t().bulkColWords];
  const rows = lastBulkResult.rows.map((row) => {
    if (!row.ok) return [String(row.line), row.raw, '', bulkErrorMessage(row)];
    const words = (lang === 'ar') ? row.arabic : row.english;
    return [String(row.line), row.raw, row.formattedWithCode, words];
  });
  downloadTextFile('bulk-conversion-results.csv', buildResultsCsv(headers, rows), 'text/csv;charset=utf-8');
}

/* ----------------------------------------------------------------------
   Print / PDF output
   ----------------------------------------------------------------------
   No PDF-generation library -- this hands off to the browser's native
   print dialog (which is also how every browser offers "Save as PDF"),
   consistent with the project's zero-runtime-dependency architecture.
   A body class scopes @media print rules (see styles.css) to show only
   the one section being printed -- the single-conversion certificate or
   the bulk report table -- and hide everything else, including the raw
   input controls that produced it.
   ---------------------------------------------------------------------- */
function printWithTarget(targetClass) {
  document.body.classList.add(targetClass);
  const cleanup = () => { document.body.classList.remove(targetClass); window.removeEventListener('afterprint', cleanup); };
...
function printResult() {
  ...
  $('certificateMeta').textContent = t().certificateMeta(currencyLabel(selectedCurrency), dateStr);
  printWithTarget('print-target-single');
}
...
function printBulkResults() {
  ...
  const dateStr = new Date().toLocaleDateString(lang === 'ar' ? 'ar' : 'en', { year: 'numeric', month: 'long', day: 'numeric' });
  $('bulkReportMeta').textContent = t().bulkReportMeta(currencyLabel(selectedCurrency), dateStr);
  printWithTarget('print-target-bulk');
}
...
$('btnBulkImportCsv').addEventListener('click', () => { $('bulkCsvFile').click(); });
$('bulkCsvFile').addEventListener('change', function () {
  const file = this.files && this.files[0];
  importBulkCsvFile(file);
  this.value = ''; // allow re-selecting the same file to re-trigger 'change'
});
$('btnBulkExportCsv').addEventListener('click', exportBulkResultsCsv);
$('btnBulkPrint').addEventListener('click', printBulkResults);
